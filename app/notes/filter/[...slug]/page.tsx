...
export async function generateMetadata({ params }: NotesFilterPageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Browse and search ${tagText} in NoteHub`,
    openGraph: {
      title: `Filter: ${tagText} | NoteHub`,
      description: `Browse and search ${tagText} in NoteHub`,
      url: `${SITE_URL}/notes/filter/${tag}`,
      images: [{ url: 'https://ac.goit.global/fullstack/react/notehub-og-meta.jpg', width: 1200, height: 630 }],
    },
  };
}
...
export default async function NotesFilterPage({ params, searchParams }: NotesFilterPageProps) {
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <NotesClient page={page} search={search} tag={isNoteTag(tag) ? tag : 'All'} />
    </HydrationBoundary>
  );
}
